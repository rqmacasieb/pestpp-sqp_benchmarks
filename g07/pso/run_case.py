"""
Run the G07 PESTPP-MOU/PSO cases built by build_case.py on this computer.

Each seed is one PESTPP-MOU run: a master plus a group of parallel workers, all
...
Run build_case.py first. You also need the compiled `pestpp-mou` binary, either
...
Usage:
    ...
    python run_case.py --exe /path/to/pestpp-mou --num_workers 8

Parallelism: each seed uses --num_workers parallel workers (default: as many as
the run can use -- mou_population_size, 20 -- but no more than your cores divided by
...
EXE_NAME = "pestpp-mou"
BASE_PORT = 15300  # seed N's master listens on BASE_PORT + N (unique per seed so concurrent seeds don't collide)
...
def workers_needed(seed):
    ...
    return int(pst.pestpp_options.get("mou_population_size", 20))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="run the G07 PESTPP-MOU/PSO cases locally")
