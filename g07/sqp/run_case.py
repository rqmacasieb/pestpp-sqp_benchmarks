"""
Run the G07 PESTPP-SQP cases built by build_case.py on this computer.

Each seed is one PESTPP-SQP run: a master plus a group of parallel workers, all
...
Run build_case.py first. You also need the compiled `pestpp-sqp` binary, either
...
Usage:
    ...
    python run_case.py --exe /path/to/pestpp-sqp --num_workers 8

Parallelism: each seed uses --num_workers parallel workers (default: as many as
the run can use -- sqp_num_reals, 20 -- but no more than your cores divided by
...
EXE_NAME = "pestpp-sqp"
BASE_PORT = 15200  # seed N's master listens on BASE_PORT + N (unique per seed so concurrent seeds don't collide)
...
def workers_needed(seed):
    ...
    return int(pst.pestpp_options.get("sqp_num_reals", 20))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="run the G07 PESTPP-SQP cases locally")
