...
equality constraints) test cases for PESTPP-OPT (sequential linear
programming with linearized chance constraints).

Each case (n = 20 and 200 decision variables) fans out into N_SEEDS
independent runs, one per starting point:
...
from ../sqp/lukvle1_{n}d/lhs_starting_values.csv if that case has already
been built there, else regenerated with the same LHS_SEED, which is
reproducibly identical) -- so seed s's PESTPP-OPT decision-variable parval1
values are exactly seed s's PESTPP-SQP starting point. Unlike PESTPP-MOU/PSO,
PESTPP-OPT (like PESTPP-SQP) perturbs a response matrix around a single
starting point rather than needing a population.
...
objective observation is zero-weighted -- a non-zero weight marks an
observation as part of PESTPP-OPT's notional calibration dataset (used to
build the FOSM J matrix), and the objective can't be both that and the
(opt_obj_func-referenced) objective. It also stays in its own "obj_fn" group:
group names with an "l_"/"g_"/"e_" prefix are reserved for constraints. The
n-2 equality constraints keep their e_constraint group.
...
"number of model runs:" line sets noptmax = round(sqp_total_runs / (n_decvars + 1)).
...
def build_seed_case(n_decvars, seed, init_vals, case_name, case_dir, noptmax):
    ...
    try:
        ...
        # the objective observation must NOT carry an "l_"/"g_"/"e_" (or
        # "less_"/"greater_"/"equal_") group prefix -- that naming is
        # reserved for PESTPP-OPT constraints -- so it stays its own,
        # unconstrained group, named explicitly via opt_obj_func below
...
def build_case(n_decvars, n_seeds=N_SEEDS, base_seed=BASE_SEED, noptmax=None):
    ...
    print("built case: {0} ({1} dec vars, {2} equality constraints, {3} seeds)".format(
        case_dir, n_decvars, n_constraints, n_seeds))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="build multi-seed LUKVLE1 N-D PESTPP-OPT cases")
