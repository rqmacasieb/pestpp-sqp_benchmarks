...
constraints). Each case is the *same problem* as the ../sqp build (same
forward model and constraints per n), so the methods are directly comparable.

Reuses ../sqp's per-case, per-seed LHS starting-value design verbatim (loaded
from ../sqp/powell_{n}d/lhs_starting_values.csv if that case has already been
built there, else regenerated with the same LHS_SEED, which is reproducibly
identical) -- so seed s's PESTPP-OPT parval1 values are exactly seed s's
PESTPP-SQP starting point.
...
reserved for constraints). An absolute derivative increment is used so it's
independent of parval1 -- an LHS-drawn start could land near 0, which would
shrink a "relative" increment toward 0 too, and PESTPP-OPT genuinely depends
...
def build_seed_case(n_decvars, seed, init_vals, case_name, case_dir, noptmax):
    ...
    try:
        ...
        # PESTPP-OPT (unlike SQP/PSO) genuinely depends on classic
        # finite-difference derinc to build its response matrix; use an
        # absolute increment so it's independent of parval1
...
def build_case(n_decvars, n_seeds=N_SEEDS, base_seed=BASE_SEED, noptmax=None):
    ...
    print("built case: {0} ({1} dec vars, {2} blocks, {3} seeds)".format(
        case_dir, n_decvars, n_decvars // 4, n_seeds))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="build multi-seed Powell N-D PESTPP-OPT cases")
