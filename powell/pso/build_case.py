...
for PESTPP-MOU (single-objective, particle swarm optimization generator).
Each case is the *same problem* as the ../sqp build (same forward model and
constraints per n), so the methods are directly comparable.
...
mou_population_size members across [-4, 5], written to
...
from ../sqp/powell_{n}d/lhs_starting_values.csv if present, else regenerated
...
The objective observation goes in a "less_than_obj" group (every
objective/constraint group needs a valid sense prefix) and is named via
mou_objectives. The constraints keep their l_/g_ groups. Unlike PESTPP-OPT
there is no FOSM machinery, so every observation keeps weight 1.0.

mou_population_size = max(20, 2n) (20, 40, 96, 200 for the four cases) --
run_case.py caps the workers at your core count.

noptmax: by default it matches that *same seed's* completed PESTPP-SQP run.
If ../sqp/powell_{n}d/seed{s}/{master,template}/powell_{n}d.rec exists, its last
...
def build_seed_case(n_decvars, seed, init_vals, case_name, case_dir, noptmax, num_reals):
    ...
    try:
        ...
        # PSO itself doesn't use derinc, but an absolute increment keeps the
        # pst consistent with the ../sqp and ../opt builds
        pst.parameter_groups.loc[:, "inctyp"] = "absolute"

        obs = pst.observation_data
        # PESTPP-MOU distinguishes objectives from constraints via
        # mou_objectives, but every objective/constraint obs group name
        # still needs a valid sense prefix ("less"/"l_" or "greater"/"g_")
...
def build_case(n_decvars, n_seeds=N_SEEDS, base_seed=BASE_SEED, noptmax=None, num_reals=None):
    ...
    print("built case: {0} ({1} dec vars, {2} blocks, mou_population_size={3}, {4} seeds)".format(
        case_dir, n_decvars, n_decvars // 4, num_reals, n_seeds))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="build multi-seed Powell N-D PESTPP-MOU/PSO cases")
