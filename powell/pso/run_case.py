"""
Run the Powell PESTPP-MOU/PSO cases built by build_case.py on this computer.
...
case powell_48d this

    powell_48d/seedN/template/  ->  copied to powell_48d/seedN/master/   (the master's
                                                              dir, and where the
                                                              results end up)
                            ->  copied to powell_48d/seedN/worker_0/ ... worker_<k-1>/
                                (removed again when the run finishes)

and writes everything the master and workers print to powell_48d/seedN/run.log.
...
Usage:
    python run_case.py 48 --seeds 1 2 3       # these seeds of the 48-variable case
    python run_case.py 48 --concurrent 4      # every built seed, 4 at a time
    python run_case.py                         # every built seed of every built case
    python run_case.py 48 --exe /path/to/pestpp-mou --num_workers 8
...
EXE_NAME = "pestpp-mou"
BASE_PORT = 5300  # seed N of an n-variable case listens on BASE_PORT + 100 * n + N (unique per case and seed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="run the Powell PESTPP-MOU/PSO cases locally")
    parser.add_argument("n_decvars", nargs="*", type=int, default=None,
                         help="which case(s) to run, e.g. 48 (default: every built case)")
