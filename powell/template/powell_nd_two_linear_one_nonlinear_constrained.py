...
# objective's own decomposition), summed over consecutive blocks -- 2 linear
# + 1 nonlinear (x1**2 term), same "square one variable in an otherwise-
# linear constraint" pattern as
# ../../rosenbrock/template/rosenbrock_2par_one_nonlinear_constrained.py
constraint1 = 0.0  # nonlinear, less-than
constraint2 = 0.0  # linear, less-than
constraint3 = 0.0  # linear, greater-than
for k in range(0, n, 4):
    x1, x2, x3, x4 = x[k], x[k + 1], x[k + 2], x[k + 3]
    constraint1 += x1 ** 2.0 + x2 - x3
