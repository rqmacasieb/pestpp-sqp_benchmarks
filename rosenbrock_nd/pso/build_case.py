...
Each case is the *same problem* as the ../sqp build.
...
from ../sqp/rosenbrock_{n}d/lhs_starting_values.csv if present, else
regenerated with the same LHS_SEED). This keeps PESTPP-SQP and
PESTPP-MOU/PSO seed-for-seed linked while still giving PSO a properly
disperse initial swarm.
...
mou_population_size = max(20, 2n) (20, 40, 100, 200 for the four cases, same
as sqp_num_reals) -- run_case.py caps the workers at your core count.
...
mou_population_size). If that record isn't found for a seed, the case's default noptmax
...
def build_case(n_decvars, n_seeds=N_SEEDS, base_seed=BASE_SEED, noptmax=None, num_reals=None):
    ...
    print("built case: {0} ({1} dec vars, {2} blocks, mou_population_size={3}, {4} seeds)".format(
        case_dir, n_decvars, n_decvars // 2, num_reals, n_seeds))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="build multi-seed Rosenbrock N-D PESTPP-MOU/PSO cases")
