"""
Run the Rosenbrock PESTPP-MOU/PSO cases built by build_case.py on this computer.

Each seed of a case is one PESTPP-MOU run: a master plus a group of parallel
workers, all started locally with pyemu.os_utils.start_workers. For seed N of
case rosenbrock_50d this

    rosenbrock_50d/seedN/template/  ->  copied to rosenbrock_50d/seedN/master/   (the master's
                                                              dir, and where the
                                                              results end up)
                            ->  copied to rosenbrock_50d/seedN/worker_0/ ... worker_<k-1>/
                                (removed again when the run finishes)

and writes everything the master and workers print to rosenbrock_50d/seedN/run.log.

Run build_case.py first. You also need the compiled `pestpp-mou` binary,
either on your PATH, next to this script, or passed with --exe.

Usage:
    python run_case.py 50 --seeds 1 2 3       # these seeds of the 50-variable case
    python run_case.py 50 --concurrent 4      # every built seed, 4 at a time
    python run_case.py                         # every built seed of every built case
    python run_case.py 50 --exe /path/to/pestpp-mou --num_workers 8

Parallelism: each seed uses --num_workers parallel workers (default: as many as
the run can use -- mou_population_size -- but no more than your cores divided by
...
EXE_NAME = "pestpp-mou"
BASE_PORT = 4300  # seed N of an n-variable case listens on BASE_PORT + 100 * n + N (unique per case and seed)
...
def workers_needed(n_decvars, seed):
    ...
    pst = pyemu.Pst(os.path.join(case_dir_for(n_decvars), "seed{0}".format(seed), "template",
                                  case_name + ".pst"))
    return int(pst.pestpp_options["mou_population_size"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="run the Rosenbrock PESTPP-MOU/PSO cases locally")
    parser.add_argument("n_decvars", nargs="*", type=int, default=None,
                         help="which case(s) to run, e.g. 50 (default: every built case)")
