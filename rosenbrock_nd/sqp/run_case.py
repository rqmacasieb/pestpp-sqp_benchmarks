"""
Run the Rosenbrock PESTPP-SQP cases built by build_case.py on this computer.

Each seed of a case is one PESTPP-SQP run: a master plus a group of parallel
...
Run build_case.py first. You also need the compiled `pestpp-sqp` binary,
...
Usage:
    ...
    python run_case.py 50 --exe /path/to/pestpp-sqp --num_workers 8

Parallelism: each seed uses --num_workers parallel workers (default: as many as
the run can use -- sqp_num_reals -- but no more than your cores divided by
...
EXE_NAME = "pestpp-sqp"
BASE_PORT = 4200  # seed N of an n-variable case listens on BASE_PORT + 100 * n + N (unique per case and seed)
...
def workers_needed(n_decvars, seed):
    ...
    pst = pyemu.Pst(os.path.join(case_dir_for(n_decvars), "seed{0}".format(seed), "template",
                                  case_name + ".pst"))
    return int(pst.pestpp_options["sqp_num_reals"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="run the Rosenbrock PESTPP-SQP cases locally")
